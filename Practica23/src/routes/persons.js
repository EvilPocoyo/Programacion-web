...
router.post('/find', (req, res) => { // Creamos una ruta para buscar a una persona
    Person.find({ nombre: { $regex: req.body.criteria, $options: "i" } }) // Buscamos a la persona por su nombre
        .then((Persons) => { res.render('person', { Persons }) }) // Regresamos la persona
        .catch((error) => { res.json({ message: error }) }); // Regresamos el error
});
